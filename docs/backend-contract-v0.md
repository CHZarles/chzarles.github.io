...
#### `POST /api/auth/device/start`（可选：给 CLI 用）

说明：用于 **GitHub Device Flow**（不依赖浏览器回跳）。适合终端/桌面客户端登录。
...
#### `POST /api/auth/device/poll`（可选：给 CLI 用）
...
```json
{
  ...
  "user": { "id": 1, "login": "charles", "avatarUrl": null },
