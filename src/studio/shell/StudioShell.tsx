...
function StudioLayout() {
  ...
  return (
    <div className="flex h-dvh flex-col bg-[hsl(var(--bg))] text-[hsl(var(--fg))]">
      <StudioHeader currentPath={location.pathname + location.search + location.hash} />
        ...
        {studio.token ? (
          ...
          <div className="mx-auto flex h-full max-w-xl flex-col justify-center px-4">
            <div className="card p-6">
              ...
              <div className="mt-1 text-sm text-[hsl(var(--muted))]">GitHub OAuth. Token stays in this browser session.</div>
...
const StudioViewport = React.memo(function StudioViewport() {
  return (
...
function StudioHeader(props: { currentPath: string }) {
  const studio = useStudioState();
  const ws = useStudioWorkspace();
  const repo = studio.me?.repo ?? null;
  const summaryPills: Array<{ label: string; accent?: boolean; mono?: boolean }> = repo
    ? [
        { label: repo.fullName, accent: true },
        { label: repo.branch },
        { label: `${ws.stats.total} staged`, accent: ws.stats.total > 0 },
      ]
    : [
        { label: "Publisher" },
        { label: `${ws.stats.total} staged`, accent: ws.stats.total > 0 },
      ...
      <div className="flex flex-col gap-3 px-4 py-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {studio.token ? (
            ...
            <button
              type="button"
              onClick={() => studio.login(props.currentPath)}
              className="inline-flex items-center gap-2 rounded-full border border-[hsl(var(--border))] bg-[hsl(var(--card))] px-3 py-2 text-sm text-[hsl(var(--muted))] transition hover:bg-[hsl(var(--card2))] hover:text-[hsl(var(--fg))]"
            >
              <LogIn className="h-4 w-4 opacity-85" />
              Login with GitHub
