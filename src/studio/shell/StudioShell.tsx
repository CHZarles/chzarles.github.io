...
function StudioLayout() {
  ...
  return (
    <div className="grid h-dvh grid-cols-1 bg-[hsl(var(--bg))] text-[hsl(var(--fg))] lg:grid-cols-[248px_minmax(0,1fr)]">
      <StudioSidebar
    ...
    <aside className="hidden min-h-0 flex-col border-r border-[color:var(--border-soft)] bg-[hsl(var(--card))] lg:flex">
      <div className="border-b border-[color:var(--border-soft)] px-5 py-5">
        <div className="min-w-0">
          ...
          <div className="mt-1 truncate text-xl font-semibold tracking-tight">Studio</div>
        </div>
        {props.repo ? (
          <div className="mt-4 space-y-1 text-xs text-[hsl(var(--muted))]">
            <div className="truncate">{props.repo.fullName}</div>
            <div className="flex flex-wrap gap-x-3 gap-y-1">
              <span>{props.repo.branch}</span>
              <span>{props.repo.headSha.slice(0, 7)}</span>
              <span>{props.stagedTotal} staged</span>
            </div>
          </div>
        ) : null}
      </div>
        ...
        {props.hasToken && props.user ? (
          <div className="flex items-center gap-3">
            {props.user.avatarUrl ? (
              ...
              <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-[color:var(--border-soft)] bg-[hsl(var(--card2))] text-[hsl(var(--muted))]">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-[color:var(--border-soft)] bg-[hsl(var(--card))] px-3 py-2 text-sm text-[hsl(var(--fg))] transition hover:bg-[hsl(var(--card2))]"
          >
...
function SidebarNavItem(props: { active: boolean; item: StudioNavItem }) {
  ...
  return (
    <NavLink
      to={props.item.to}
      className={[
        "flex items-center justify-between border-l-2 px-3 py-2.5 transition",
        props.active
          ? "border-l-[hsl(var(--accent))] bg-[hsl(var(--bg))] text-[hsl(var(--fg))]"
          : "border-l-transparent text-[hsl(var(--muted))] hover:bg-[hsl(var(--card2))] hover:text-[hsl(var(--fg))]",
      ].join(" ")}
    >
      <div className="flex min-w-0 items-center gap-3">
        <Icon className={["h-4 w-4 shrink-0", props.active ? "text-[hsl(var(--fg))]" : "text-[hsl(var(--muted))]"].join(" ")} />
        <div className="truncate text-sm font-medium tracking-tight">{props.item.label}</div>
      </div>
      {props.item.badge ? <NavCount value={props.item.badge} active={props.active} /> : null}
    </NavLink>
  );
}
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-sm text-[hsl(var(--muted))]">
                <span>{props.currentSection.description}</span>
                {props.repo ? <span>{props.repo.branch}</span> : null}
                <span>{props.stats.total} staged</span>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <button
                ...
                className={[
                  ...
                  !props.stagedTotal || props.publishing
                    ? "cursor-not-allowed border border-[hsl(var(--border))] bg-[hsl(var(--bg))] text-[hsl(var(--muted))]"
                    : "border border-[color-mix(in_oklab,hsl(var(--accent))_40%,hsl(var(--border)))] bg-[hsl(var(--card))] text-[hsl(var(--fg))] hover:bg-[hsl(var(--bg))]",
                ].join(" ")}
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-[color:var(--border-soft)] bg-[hsl(var(--card))] px-3 py-2 text-sm text-[hsl(var(--muted))] transition hover:bg-[hsl(var(--bg))] hover:text-[hsl(var(--fg))]"
              >
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-[hsl(var(--muted))]">
            <span>Notes {props.stats.notes}</span>
            <span>Config {props.stats.config}</span>
            <span>Assets {props.assetOps}</span>
            {props.lastCommitUrl ? (
              <a href={props.lastCommitUrl} target="_blank" rel="noreferrer" className="hover:text-[hsl(var(--fg))]">
                Latest commit
              </a>
            ) : null}
          </div>
    ...
    <div
      className={[
        ...
        props.notice.tone === "error"
          ? "border-l-red-600 bg-[color-mix(in_oklab,red_4%,transparent)] text-red-700"
          : "border-l-orange-500 bg-[color-mix(in_oklab,orange_4%,transparent)] text-[hsl(var(--fg))]",
      ].join(" ")}
    >
...
function NavCount(props: { value: number; active?: boolean }) {
  return (
    <span
      className={[
        "inline-flex min-w-5 items-center justify-center px-1 text-[11px] font-medium tracking-tight",
        props.active
          ? "text-[hsl(var(--fg))]"
          : "text-[hsl(var(--muted))]",
      ].join(" ")}
